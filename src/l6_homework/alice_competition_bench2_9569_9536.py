...
def assignment6_baseline(features):
  ...
  print('------------- training -------------')
  predictor, score = get_auc_lr_valid(X_train, y_train)

  print('------------- saving results -------------')
  y_test = predictor.predict_proba(X_test)[:, 1]
  write_to_submission_file(y_test, 'assignment6_alice_submission.csv')
